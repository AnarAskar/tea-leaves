...
function formatUploadError(error) {
  ...
  if (/bucket.*not found|not found.*bucket/i.test(msg)) {
    return "Storage bucket not set up. Run supabase/setup_category_images.sql in the Supabase SQL Editor.";
  }
  if (/row-level security|policy/i.test(msg)) {
    return "Upload blocked by storage permissions. Re-run supabase/setup_category_images.sql, then sign out and back in.";
  }
  if (/mime|invalid.*type/i.test(msg)) {
    return `${msg} Try saving as JPEG or PNG, or re-run supabase/setup_category_images.sql.`;
  }
  return msg;
}
